...
void updateOrbiters(double timeDelta) {
  updateOrbiters(timeDelta, sun);
}

void updateOrbiters(double timeDelta, Orbiter origin) {
  
  origin.updateRotation(timeDelta);
  
  ArrayList<Orbiter> children = new ArrayList<Orbiter>(origin.getChildren());
  
  for (int i = 0; i < children.size(); i++) {
    updateOrbiters(timeDelta, children.get(i));
  }
...
void drawOrbiters(Orbiter origin) {
